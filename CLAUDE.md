...
- **Pop out** (`components/session/FloatingTimer.tsx`, provider mounted in `(app)/layout.tsx` so it survives in-app navigation): Chrome/Edge → Document Picture-in-Picture, an always-on-top window you drag anywhere/resize, rendered by portal (`MiniTimer`, clock scales with vw/vh; page stylesheets cloned in). Other browsers → `window.open('/timer/[id]')` (route outside `(app)`, no nav). "⧉ Pop out" / "⤢ Dock timer" on `/session/[id]`.
