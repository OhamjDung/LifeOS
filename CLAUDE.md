...
- **State lives in `web/lib/useFocusSession.ts`** (load, 1s tick + one exact end-time timeout so background-tab throttling can't delay the chime, pause/resume/break/next-round, round rating, tab-title countdown). Instances in other windows sync via `BroadcastChannel('lifeos-session-<id>')`.
- **Chime** (`web/lib/chime.ts`): Web Audio bell, no asset. Fires once when a *running* phase crosses to 0 (never on load), deduped across windows via localStorage key `chimed:<id>:<phase>:<phase_started_at>`; plus a desktop Notification if permitted and the tab is hidden. 🔔/🔕 toggle on the session page (localStorage `timerSound`; turning it on previews the sound + asks notification permission). Audio unlocks on the first pointerdown (browser gesture rule).
- **Pop out** (`components/session/FloatingTimer.tsx`, provider mounted in `(app)/layout.tsx` so it survives in-app navigation): Chrome/Edge → Document Picture-in-Picture, an always-on-top window you drag anywhere/resize, rendered by portal (`MiniTimer`, clock scales with vw/vh; page stylesheets cloned in). Other browsers → `window.open('/timer/[id]')` (route outside `(app)`, no nav). "⧉ Pop out" / "⤢ Dock timer" on `/session/[id]`. The pop-out has tabs **TIMER | SESSION** (`SessionTaskPanel`) **| TASKS** (`MiniTaskList`: today+overdue pending + done today, tick/quick-add, fires `DATA_CHANGED`) **| TODAY** (`DayCalendar compact`); tab bar is phase-coloured and shows the countdown off the TIMER tab; last tab in localStorage `popoutTab`. Chime keeps working on every tab (the `useFocusSession` instance lives in the host, not the tab). **Theme follows the phase**: work → the whole window gets `.theme-dark`, break/idle → light. Also openable **without a session** via `PopOutButton` on the `/tasks` header: attaches to the latest active session, else opens with TASKS/TODAY + a "start focus session" face (resume an active one or new 25/5·50/10·90/15; `createFocusSession()`); fallback popup route is `/timer/none`.
...
web/app/
  ...
  (app)/session/[id]/page.tsx    # focus session timer — phase-colored full-screen, SessionTaskPanel, End/Lockin modals, 🔔 chime toggle, ⧉ Pop out (client component, useParams)
  timer/[id]/page.tsx            # bare pop-out timer (outside (app): no nav) — fallback when Document PiP isn't supported
...
- **Colors are a non-inline `@theme` in globals.css** (fonts stay in `@theme inline`), so utilities compile to `var(--color-*)`. `.theme-dark` overrides those vars for a subtree (inverted warm palette) — used by the pop-out during work phases. Calendar inline colors use `--cal-*` vars for the same reason. Input surface token: `bg-field` (#EAE7E0 light / #2F2B24 dark) — prefer it over `bg-[#EAE7E0]`.
...
- `contacts.category` (v14): `family|work|friend|other` (nullable). `ContactCategoryPicker` chips on /contacts cards (compact) + /contacts/[id] header; picker on /contacts/new; the chat sets it via `upsert_contact`.
...
- **Tools** (`_shared/chatTools.ts`): READ run immediately (`list_tasks`, `get_task`, `search_notes`, `list_contacts`, `get_contact`, `get_calendar`, `get_plan`, `remember`). WRITE never touch data — they become **proposals** stored on the assistant `chat_messages.proposals` (`create_tasks` w/ Jina near-dup warning, `update_tasks` (rename/reschedule/★/done), `delete_tasks`, `add_subtasks`, `upsert_contact` (category family/work/friend/other; name collision → `choices` the user picks), `create_note`, `schedule_block`). `{action:'apply'}` loads proposals by id from the DB (client never sends args — and v15 makes `chat_threads`/`chat_messages` **select+delete only** for users, so stored proposals can't be tampered with before the service-role apply; `update_tasks` apply also whitelists columns), re-validates, applies, marks `applied|rejected|failed` — idempotent. Planner is read-only for the chat by design; no session tools.
