...
export default async function TasksPage({
  ...
  // Fetch today's tasks with tags for TaskList
      ...
      <div className="w-1/2 p-6 overflow-y-auto min-w-0">
        ...
        <div className="grid grid-cols-7 mb-1">
          {['S','M','T','W','T','F','S'].map((lbl, i) => (
            <div key={i} className="text-center text-xs text-gray-600 py-1 font-medium">{lbl}</div>
          ))}
        </div>
        ...
        <div className="border border-gray-800 rounded-xl overflow-hidden">
          {weeks.map((week, wi) => (
            <div key={wi} className={`grid grid-cols-7 ${wi < weeks.length - 1 ? 'border-b border-gray-800' : ''}`}>
              {week.map((day, di) => {
                if (!day) return (
                  <div key={di} className={`min-h-16 p-1 bg-gray-950 ${di < 6 ? 'border-r border-gray-800' : ''}`} />
                )
                const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`
                ...
                return (
                  ...
                  >
                    ...
                    {events.slice(0, 1).map(t => (
                      <div
                        key={t.id}
                        title={t.title}
                        className={`text-[10px] px-1 py-0.5 rounded mb-0.5 truncate ${
                          t.status === 'done' ? 'bg-gray-800 text-gray-500 line-through' : 'bg-indigo-900/50 text-indigo-300'
                        }`}
                      >
                        {t.title}
                      </div>
        ...
        {/* Selected day panel */}
        {selectedDate && (
          <div className="mt-4 rounded-xl p-4" style={{ background: '#DEDAD2', border: '1px solid rgba(28,26,20,0.1)', boxShadow: '2px 2px 6px rgba(107,99,88,0.1)' }}>
            <h3 className="font-medium text-sm mb-3" style={{ color: '#1C1A14' }}>{selectedLabel}</h3>
            {selectedTasks.length === 0 ? (
              <p className="text-xs" style={{ color: '#837C6F' }}>Nothing scheduled.</p>
            ) : (
              <div className="space-y-1.5">
                {selectedTasks.map(t => (
                  <div key={t.id} className="flex items-center gap-2">
                    <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                      t.status === 'done' ? 'bg-gray-600' : t.task_type === 'event' ? 'bg-indigo-400' : 'bg-indigo-500'
                    }`} />
                    <span className={`text-xs ${t.status === 'done' ? 'line-through text-gray-500' : ''}`} style={{ color: t.status === 'done' ? undefined : '#1C1A14' }}>
